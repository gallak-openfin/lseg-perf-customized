...
console.log(
  "Initializing Ten Windows Container Provider (@openfin/core only)...",
);
...
// limits concurrent operations to avoid saturating the browser process event loop
// when snapshots or parallel calls fire many windows at once.
const makeConcurrencyQueue = (maxConcurrent) => {
  let running = 0;
  const pending = [];
  return {
    acquire: () => {
...
async function initializePlatform() {
  ...
  try {
    ...
    // TODO: Uncomment below to turn "on" staggering by 2 windows at a time
    // await fin.Platform.init({
    ...
    //         const { reason = "api-call" } = options;
    //         if (
    //           reason !== "apply-snapshot" ||
    //           !options.experimental?.staggerDuringApplySnapshot
    //         ) {
    //           // standard platform.createWindow call OR experimental flag off, ignore the stagger mechanism
    ...
    //         console.log(
    //           `>>> applySnapshot start, payload: ${JSON.stringify(payload, null, 2)}`,
    //         );
    //         // shim our experimental option on all snapshot windows
    //         for (let i = 0; i < payload.snapshot.windows.length; i++) {
    //           payload.snapshot.windows[i].experimental ??= {};
    //           payload.snapshot.windows[i].experimental.staggerDuringApplySnapshot = true;
    //         }
    //         console.log(
    //           `>>> augmented snapshot: ${JSON.stringify(payload, null, 2)}`,
    //         );
      ...
      try {
        switch (message.action) {
          ...
          case "set-delay":
            ...
            break;
          case "launch-windows":
            await testModules.windows.handleLaunchWindows(
              identity,
              testState,
              sendStatus,
              { content: "site" },
          ...
          case "launch-browser-windows":
            await testModules.browserWindows.handleLaunchBrowserWindows(
              identity,
              testState,
              sendStatus,
              { content: "site" },
          ...
          case "launch-layout-windows":
            ...
            break;
          case "apply-custom-snapshot":
            await testModules.browserWindows.handleApplyCustomSnapshot(
              identity,
              testState,
              sendStatus,
            );
            break;
          case "close-windows":
            await testModules.closeWindows.handleCloseWindows(
              identity,
              testState,
              sendStatus,
            );
...
if ("fin" in window && window.fin !== null && typeof window.fin === "object") {
  ...
  window.addEventListener("finReady", initializePlatform);
}
