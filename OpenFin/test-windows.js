// Regular Window Test - Creates 10 regular OpenFin windows ----------- blah!

async function handleLaunchWindows(
  targetIdentity,
  testState,
  sendStatus,
  { content },
  ...
  sendStatus(targetIdentity, {
    type: "info",
    message: "Starting windows creation...",
  });
  ...
  try {
    ...
    const makePromise = (i) => {
      ...
      sendStatus(targetIdentity, {
        type: "info",
        message: `Creating window ${i + 1}: ${windowName} (${url})...`,
      });
      ...
      const loadedPromise = window.windowUtils.waitForWindowLoaded(
        win,
        windowName,
      );
      ...
      loadedPromise.then(() => {
        sendStatus(targetIdentity, {
          type: "success",
          message: `✓ Window ${windowName} loaded successfully (${url})`,
        });
      });
      ...
      createdPromise.then(() => {
        sendStatus(targetIdentity, {
          type: "success",
          message: `✓ Window ${windowName} created successfully (${url})`,
        });
      });
    ...
    sendStatus(targetIdentity, {
      type: "error",
      message: `❌ Error creating windows: ${error.message}`,
    });
  } finally {
