  ...
  try {
    ...
    const platform = await WorkspacePlatform.init({
      browser: {
        ...
        browserBaseUrl:
          "https://workspace.openfin.co/workspace/pr/g/lseg-22.3.29-perf-MTP-layouts/system-apps/workspace",
        // browserBaseUrl:
        //   "https://workspace.openfin.co/workspace/pr/g/lseg-22.3.29-perf-MTP-layouts-no-sw/system-apps/workspace",
      },
      // workspaceAsar: {
      //   path: `/Users/george/code/scratch/lseg-perf/OpenFin/workspace.asar`,
      // },
...
function setupIPCHandlers() {
  ...
  fin.InterApplicationBus.subscribe(
    { uuid: "*" },
    "ipc-relay-to-window2",
    async (message, identity) => {
      console.log(
        "IPC Relay: Received message from window1, forwarding to window2"
      );
  ...
  fin.InterApplicationBus.subscribe(
    ...
    async (results, identity) => {
      ...
      fin.InterApplicationBus.send(
        ...
        console.error("Error sending results to launcher:", err)
      );
    }
  );
}
