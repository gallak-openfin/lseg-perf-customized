...
const Perfs = {
  ...
  startOpen: (testName, count, delay) => {
    ...
    console.log(
      `[${now.display}] START opening "${testName}" (${count} windows)`
    );
    Perfs._data = {
      testName: `Opening ${count} ${testName}`,
      count,
      delay,
      measures: {},
      summary: {},
  ...
  startClose: (count) => {
    const now = Perfs._now();
    Perfs._data = {
      testName: `Closing ${count} Windows`,
      count,
      measures: {},
      summary: {},
    };
  ...
  created: (windowName) => {
    ...
    console.log(
      `[${now.display}][+${pad(time)}] Window Created: ${windowName}`
    );
  },
  ...
  windowEvent: (windowName, eventName) => {
    ...
    console.log(
      `[${now.display}][+${pad(
        time
      )}] Event: ${windowName} - window:${eventName}`
    );
    measure.loaded = time;
  },
  ...
  viewEvent: (windowName, eventName) => {
    ...
    console.log(
      `[${now.display}][+${pad(time)}] Event: ${windowName} - view:${eventName}`
    );
    measure.loaded = time;
  },
  ...
  log: (windowName, eventName) => {
    ...
    const time = now.time - measure.origin;
    console.log(
      `[${now.display}][+${pad(time)}] Log: ${windowName} - ${eventName}`
    );
  },
  ...
  _computeStats: () => {
    ...
    const created = measuresArray
      .map((measure) => measure.created)
    ...
    if (created.length) {
      ...
      summary.push(
        `${data.testName} - Time to create ${created.length} windows: total ${sumCreated} ms (avg: ${avgCreated} ms)`
      );
      data.summary.totalCreationTime = maxCreated;
      data.summary.averageCreationTime = avgCreated;
    }

    const loaded = measuresArray
      .map((measure) => measure.loaded)
      .filter((v) => v !== undefined);
    if (loaded.length) {
      ...
      summary.push(
        `${data.testName} - Time to load ${loaded.length} windows: total ${sumLoaded} ms (avg: ${avgLoaded} ms)`
      );
      data.summary.totalLoadingTime = maxLoaded;
      data.summary.averageLoadingTime = avgLoaded;
    }

    const closed = measuresArray
      .map((measure) => measure.closed)
      .filter((v) => v !== undefined);
    if (closed.length) {
      ...
      summary.push(
        `${data.testName} - Time to close ${closed.length} windows: total ${sumClosed} ms (avg: ${avgClosed} ms)`
      );
