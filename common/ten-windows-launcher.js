...
function setupStatusListener() {
  ...
  platformAPI.addListener((message) => {
    ...
    if (message.completed) {
      ...
      document.getElementById("applyCustomSnapshotBtn").disabled = false;
...
async function executeLaunchCommand(
  buttonId,
  action,
  statusMessage,
  commandName,
  ...
  try {
    ...
    updateStatus(
      `❌ Error sending ${commandName} command: ${error.message}`,
      "error",
    );
...
async function onApplyCustomSnapshot() {
  await executeLaunchCommand(
    "applyCustomSnapshotBtn",
    "apply-custom-snapshot",
    "Sending custom snapshot apply command...",
    "custom snapshot apply",
  );
}
...
async function init() {
  try {
    ...
    if (
      ...
      window.location.search.includes("platform=container")
    ) {
